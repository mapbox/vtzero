...
TEST_CASE("geometry_decoder") {
    ...
    vtzero::detail::geometry_decoder<iterator> decoder{g.cbegin(), g.cend(), true, g.size() / 2};
...
TEST_CASE("geometry_decoder with incomplete point") {
    ...
    vtzero::detail::geometry_decoder<iterator> decoder{g.cbegin(), g.cend(), true, 100};
    REQUIRE(decoder.count() == 0);
    REQUIRE_FALSE(decoder.done());
...
TEST_CASE("geometry_decoder with linestring with equal points") {
    ...
    vtzero::detail::geometry_decoder<iterator> decoder{g.cbegin(), g.cend(), strict, g.size() / 2};
    REQUIRE(decoder.count() == 0);
    REQUIRE_FALSE(decoder.done());
...
TEST_CASE("geometry_decoder with multipoint with not enough points") {
    ...
    vtzero::detail::geometry_decoder<iterator> decoder{g.cbegin(), g.cend(), true, 1};
    REQUIRE(decoder.count() == 0);
    REQUIRE_FALSE(decoder.done());
