...
TEST_CASE("Calling decode_point_geometry() with empty input") {
    ...
    REQUIRE_THROWS_AS(vtzero::detail::decode_point_geometry(g.cbegin(), g.cend(), true, dummy_geom_handler{}),
                      const vtzero::geometry_exception&);
    REQUIRE_THROWS_WITH(vtzero::detail::decode_point_geometry(g.cbegin(), g.cend(), true, dummy_geom_handler{}),
...
TEST_CASE("Calling decode_point_geometry() with a valid point") {
    ...
    dummy_geom_handler handler;
    vtzero::detail::decode_point_geometry(g.cbegin(), g.cend(), true, handler);
...
TEST_CASE("Calling decode_point_geometry() with a valid multipoint") {
    ...
    REQUIRE(vtzero::detail::decode_point_geometry(g.cbegin(), g.cend(), true, dummy_geom_handler{}) == 10201);
}
